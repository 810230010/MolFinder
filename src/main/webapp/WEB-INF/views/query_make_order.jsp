        ...
        <fieldset>
            ...
            <div class="row">
                <table class="table table-bordered">
                    ...
                    <tr>
                        <th><fmt:formatDate value="${queryCallpriceDetail.createTime}" pattern="yyyy-MM-dd"/></th>
                        <th>${queryCallpriceDetail.casNo}</th>
                        <th>${queryCallpriceDetail.chineseName}(${queryCallpriceDetail.englishName})</th>
                        <th colspan="2">${queryCallpriceDetail.callPriceMoney}</th>
                    </tr>
                    </thead>
                    <tbody>
                    <tr>
                        <c:set value="${ fn:split(queryCallpriceDetail.callPriceMoney, '/') }" var="callItems" />
                        <td>${callItems[1]}</td>
        ...
        <fieldset>
            ...
            <div class="row">
                ...
                <div class="form-group">
                    <label class="col-sm-3 control-label">CAS号: <u>${queryCallpriceDetail.casNo}</u></label>
                </div>
                <div class="form-group">
                        <label class="col-sm-12 control-label">
                            开票类型:
                            <u>${queryCallpriceDetail.callPriceMoney}(${queryCallpriceDetail.makeBill})</u>
                        </label>

                </div>
                <div class="form-group">
                    <label class="col-sm-12 control-label">纯度: <u>${queryCallpriceDetail.callPurity}%</u></label>
                </div>
                <div class="form-group">
                    <label class="col-sm-12 control-label">快递方式: <u>${queryCallpriceDetail.expressType}</u></label>
                </div>
                <div class="form-group">
                    <label class="col-sm-12 control-label">交货期: <u>${queryCallpriceDetail.callSubmitDeadline}</u></label>
                </div>
                <div class="form-group">
                    <label class="col-sm-1 control-label">提供图谱:</label>
                    <div class="col-sm-11">
                        <c:set value="${ fn:split(queryCallpriceDetail.offerDiagram, ',') }" var="names" />
                            ...
                            <div class="panel panel-default">
                                ...
                                <div id="collapseOne" class="panel-collapse collapse in">
                                    <div class="panel-body">
                                        <p>可付订单价<input id="content1" type="text" style="width: 60px;display:inline" value="${queryCallpriceDetail.prepayedMoneyAmount}" readonly>%至网站作保证金(默认0.5%)</p>
                                        <p>接受违约金过期<input id="content2" type="text" style="width: 60px;display: inline" value="${queryCallpriceDetail.vilationMoneyAmount}" readonly>%/天(默认1%)</p>
                                    </div>
                                </div>
                            </div>
    ...
    $(document).ready(function(){
        ...
        $("#form").steps({
            ...
            onFinished: function (event, currentIndex)
            {
                ...
                var price = "${queryCallpriceDetail.callPriceMoney}";
                var total = price.substring(0, indexOf("元"));
                $.ajax({
                    url: "/order/makeOrderBill",
                    data:{
                        buyerId: ${currentUser.userId},
                        callPriceId: ${queryCallpriceDetail.queryCallId},
                        expressWay: ${queryCallpriceDetail.expressType},
                        remark: $("#remark").val(),
                        orderType: "QUERY",
