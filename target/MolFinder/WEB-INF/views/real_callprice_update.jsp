                ...
                <form class="form-horizontal" id="form" onsubmit="return false">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group col-sm-7" style="position:relative;left: 20px">
                            <c:set value="${ fn:split(realCallpriceDetail.callPriceMoney, ',') }" var="priceList" />
                            <c:set value="${ fn:split(realCallpriceDetail.callPriceAmount, ',') }" var="amountList" />
                            <c:forEach var="price" items="${priceList}" varStatus="idx">
                                <c:set var="index" value="${idx.index}"/>
                                <c:set var="amount" value="${ fn:substring(price, 0, fn:indexOf(price, '元')) }"/>
                                <span style="margin-top:30px">
                                    <input class="input-sm price" type="text" name="lowPrice" value="${amount}">元/
                                    <input class="input-sm" type="text" name="highPrice" value="${amountList[index]}" readonly>
                                </span>
                            </c:forEach>
                            ...
                            <div class="panel-group" id="accordion">
                                <div class="panel panel-default">
                                    ...
                                    <div class="panel-body">
                                        <p>可付订单价<input id="content1" type="text" style="width: 60px" value="${realCallpriceDetail.prepayedMoneyAmount}">%至网站作保证金(默认0.5%)</p>
                                        <p>接受违约金过期<input id="content2" type="text" style="width: 60px" value="${realCallpriceDetail.vilationMoneyAmount}">%/天(默认1%)</p>
                                    </div>
                                </div>
                                </div>
                    ...
                    <div class="form-group">
                        <div class="text-center">
                            <input id="submit" type="button" class="btn btn-primary" value="修改报价" onclick="updateRealCallprice()"/>
                        </div>
                    </div>
                </form>
            </div>
...
<script>
    ...
    $(function(){
        ...
        var bill = "${realCallpriceDetail.makeBill}";
        $('.bill').val(bill).trigger('change');
    });


    //跳转到下单页面
    function updateRealCallprice() {
        var callPriceMoney = "";
        $(".price").each(function (index, value) {
            if(index == $(".price").length-1)
                callPriceMoney += $(this).val() + "元"
            else
                callPriceMoney += $(this).val() + "元,"
        })
        $.ajax({
            url: "/real/updateRealOrderCallprice",
            data:{
                realCallId: ${realCallpriceDetail.realCallId},
                realOrderId: ${realCallpriceDetail.realOrderId},
