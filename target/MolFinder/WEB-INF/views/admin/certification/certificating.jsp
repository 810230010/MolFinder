...
<script>
    $('#datatable').DataTable({
        "ajax": {
            'url': '/admin/certification/listCertificating',
            "data": function(d) {
                var param = {};
        ...
        "columns": [
            {"data":"certificateId","width":"7%","title":"","visible": false},
            {"data":"phone","width": "10%","title":"认证用户","orderable": false},
            {"data":"certificateType","width": "10%","title":"认证类型","orderable": false},
            {"data":"realname","width": "10%","title":"真实姓名","orderable": false},
            {"data":"pass","width": "10%","title":"审核状态","orderable": false},
            {"data":"createTime1","width": "10%","title":"申请时间","orderable": false},
            {
                "data":"userId",
                "width": "8%",
                "title":"操作",
                ...
                "render": function (data, type, row) {
                    return [
                        '<a class="btn btn-primary btn-xs table-action btn-block detail">',
                        '查看详情 <i class="fa fa-eye"></i>',
                        '</a>',
                        '<a class="btn btn-success btn-xs table-action btn-block pass">',
                        '通过 <i class="fa fa-flag"></i>',
                        '</a>',
                        '<a class="btn btn-danger btn-xs table-action btn-block notpass">',
    ...
    table.on( 'click', '.detail', function () {
        ...
        window.location.href = "/admin/certification/certificationDetail.html?certificateId=" + data.certificateId;
    });
    // 通过
    table.on( 'click', '.pass', function () {
        ...
        var data = table.row(tr).data();
        $.ajax({
            url: '/admin/certification/pass',
            data: {
                certificateId: data.certificateId
            },
            success: function (result) {
                if(result.code == 200){
                    swal("成功！", "已通过该用户实名认证申请！","success");
                    setTimeout(function () {
    ...
    // 不通过
    table.on( 'click', '.notpass', function () {
        var tr = $(this).closest('tr');
        var data = table.row(tr).data();
        swal({
                title: "确定不通过吗？",
            ...
            function(){
                $.ajax({
                    url: '/admin/certification/notpass',
                    data: {
                        certificateId: data.certificateId
                    },
                    success: function (result) {
                        if(result.code == 200){
                            swal("成功！", "已拒绝该用户实名认证申请！","success");
                            setTimeout(function () {
                                window.location.reload();
                            }, 1500)
                        }
